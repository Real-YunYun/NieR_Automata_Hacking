...
namespace Entities {
    ...
    public class MovementComponent : EntityModule {

        [Header("Movement Parameters")] 
        public MovementController MovementControllerType;
        ...
        private void OnEnable() {
            if (!Owner.TryGetComponent(out StatsComponent Stats)) return;
            Stats.OnStatsChanged += OnStatsChanged;
        }

        public void OnStatsChanged(StatsComponent Comp) {
            MoveSpeed = Comp.GetStatResult(StatFactory.MoveSpeed);
        }
        ...
        public virtual void Move(Vector2 Direction2D) {
            if (!TryGetComponent(out CharacterController Controller)) return;
            ...
            if (Direction3D.magnitude >= 0.1f) {
                ...
                Vector3 MoveDir = Quaternion.Euler(0f, TurningAngle, 0f) * Vector3.forward;
